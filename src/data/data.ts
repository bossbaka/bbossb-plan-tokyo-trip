import KamakurakokoMaeImage from "../assets/tripp-ho-3mGGMau_4c8-unsplash.jpg";
import KurazukurinoMachinamiImage from "../assets/kentaro-toma-ZSszWRLLlbg-unsplash.jpg";
import KawagoeHikawaShrineImage from "../assets/letian-zhang-RGsMFx-9Md0-unsplash.jpg";
import ShingashiRiverbankImage from "../assets/nguyen-tp-hai-Mq9mdogzFBs-unsplash.jpg";
import OishiParkCafeImage from "../assets/owen-roth-TPycWrCxxsc-unsplash.jpg";
import NakaMeguroStaImage from "../assets/pexels-ronny-siegel-11638591.jpg";
import GalaYuzawaImage from "../assets/Snow-Enjoyment-Park.jpg";
import SuvarnabhumiAirportImage from "../assets/pexels-markus-winkler-4974985.jpg";

const swiperData = [
  {
    day: "Saturday, 23 Mar. 2024",
    fromtime: "18:00",
    planfrom: "ออกเดินทางจากวิหารแดง",
    duration: "ใช้เวลา 2 - 3 ชั่วโมง",
    planto: "ไปถึงสนามบินสุวรรณภูมิ",
    totime: "21:00 - 22:00",
    detailmore: `
    <a href="https://www.google.com/maps/search/?api=1&query=Suvarnabhumi+Airport">
    <div><span class="tag-time">เวลา 18:00</span>
    ออกเดินทางจากวิหารแดง &nbsp;
    <iconify-icon icon="ph:car" width="30" height="30"></iconify-icon>
    <span class="tag-duration">เดินทางประมาณ 2 - 3 ชั่วโมง</span> 
    ถึงสนามบินสุวรรณภูมิ <span class="tag-time">เวลา 21:00 - 22:00</span></div>
    </a>

    <hr class="line">

    <div>Check in Counter AirJapn 
    สามารถ Check In ได้ก่อนล่วงหน้า 3 ชั่วโมง ควร 
    <span class="tag-duration">Check In ตอน 21:00</span></div> 
    
    <hr class="line">
    
    <div>เข้าด่านตรวจ 
    <iconify-icon icon="mdi:account-tie-hat-outline" width="30" height="30"></iconify-icon>
    </div>
    
    <hr class="line">
    
    <div>ไป Gate รอเตรียมขึ้นเครื่องบิน </div>
   
    <hr class="line">
   
    <div>
    เครื่องบินออก 
    <iconify-icon icon="mdi:airplane-takeoff" width="30" height="30"></iconify-icon>
    <span class="tag-time">00:15 น. วันอาทิตย์ที่ 24 </span>
    </div>
    `,
    locations: [
      {
        location_name: "Suvarnabhumi Airport",
        picture: SuvarnabhumiAirportImage,
        path: "/locations/suvarnabhumi_airport",
      },
    ],
  },

  /* ------------------------------------------------- 24 ------------------------------------------------------ */

  {
    day: "Sunday, 24 Mar. 2024",
    fromtime: "00:15",
    planfrom: "อยู่บนเครื่องบิน AirJapan NQ002",
    duration: "เดินทางประมาณ 5 ชั่วโมง 55 นาที",
    planto: "ถึงสนามบินนาริตะ Narita International",
    totime: "08:10",
    detailmore: `
    <div>
    <span class="tag-time">เวลา 00:15</span>
    อยู่บนเครื่องบิน AirJapan NQ002
    <iconify-icon icon="mdi:seat-passenger" width="30" height="30"></iconify-icon>
    <span class="tag-duration">เดินทางประมาณ 5 ชั่วโมง 55 นาที</span> 
    ถึงสนามบินนาริตะ 
    <a href="https://www.narita-airport.jp/en/map" target="_blank" class="detail-map">
    Narita International  
    </a>
    <span class="tag-time">เวลา 08:10</span>
    </div>

    <hr class="line">

    <div>เข้าด่านตรวจคนเข้าเมือง Immigration</div> 
    
    <hr class="line">
   
    <div>
    ไปหยิบกระเป๋าที่รางสายพาน  
    <iconify-icon icon="mdi:bag-checked" width="30" height="30"></iconify-icon>
    </div>
   
    <hr class="line">
   
    <div>
    เข้าด่านกรมศุลกากร Customs &nbsp;
    <iconify-icon icon="fluent-emoji-high-contrast:customs" width="30" height="30"></iconify-icon>
    </div>
    
    <hr class="line">
   
    <a href="https://www.jreast.co.jp/e/customer_support/service_center.html" target="_blank">
    <div><i>ซื้อ IC Card Welcome Suica และเติมเงินเข้า IC Card</i></div>
    </a>
   
    <hr class="line">
    
    <div>
    <span class="tag-time">เวลา 11:22</span>
    ขึ้นรถไฟสถานี 
    <a href="https://www.narita-airport.jp/en/access/train" target="_blank" class="detail-map">
    Narita Station 
    </a>
    <span class="tag-line ks">สาย Keisei Narita Skyaccess</span> 
    ต่อบนพาหนะเดิม
    <span class="tag-line a">สาย Asakusa Line</span>
    <span class="tag-duration">เดินทางประมาณ 1 ชั่วโมง 19 นาที</span> 
    ถึงสถานี 
    <a href="https://www.tokyometro.jp/lang_th/station/nihombashi/index.html" target="_blank" class="detail-map">
    Nihombashi Station 
    </a>
    <span class="tag-time">เวลา 12:29</span>
    <a href="https://www.google.com/maps/dir/?api=1&origin=Narita+Airport+Terminal+1&destination=Nihombashi+Station&travelmode=transit" class="open-map">
    เปิดเส้นทาง GoogleMap
    </a>
    </div>

    <hr class="line">
    
    <div>เดิน 
    <span class="tag-duration"> 40 นาที 750 เมตร </span> 
    <iconify-icon icon="mdi:bag-suitcase-outline" width="30" height="30"></iconify-icon>
    <a href="https://www.sagawa-exp.co.jp/ttk/english/servicecenter/tokyo_station.html" target="_blank">
    <i>ฝากกระเป๋าที่ SAGAWA EXPRESS TOKYO SERVICE CENTER ร้านปิด 21:00</i> 
    </a>
    <span class="tag-time">เวลา 13:09</span>
    <a href="https://www.google.com/maps/search/?api=1&query=SAGAWA+EXPRESS+TOKYO+SERVICE+CENTER" class="open-map">
    เปิดเส้นทาง GoogleMap
    </a>
    </div>
    
    <hr class="line">
    
    <a href="https://www.google.com/maps/search/?api=1&query=food+Tokyo+Station">
    <div>
    <i>ทานอาหาร </i>
    <iconify-icon icon="fluent:food-20-regular" width="30" height="30"></iconify-icon>
    <span class="tag-duration">ประมาณ 1 ชั่วโมง </span> 
    <span class="tag-time">เวลา 14:09</span></div>
    </a>

    <hr class="line">
    
    <div>เดินไปขึ้นรถไฟ
    <iconify-icon icon="tabler:walk" width="30" height="30"></iconify-icon>
    <iconify-icon icon="ph:train" width="30" height="30"></iconify-icon>
    <span class="tag-duration">ประมาณ 20 นาที </span> 
    <span class="tag-time">เวลา 14:20</span> </div>
    
    <hr class="line">
    
    <div>
    <span class="tag-time">เวลา 14:38</span>
    ขึ้นรถไฟสถานี 
    <a href="https://www.jreast.co.jp/e/stations/e1039.html" target="_blank" class="detail-map">
    Tokyo Station 
    </a>
    <span class="tag-line jo">สาย Yokosuka </span>
    <a href="https://www.jreast.co.jp/e/stations/e476.html" target="_blank" class="detail-map">
    แผนที่ Kamakura Station ต่อขบวน ไปขึ้นสาย
    </a>
    <iconify-icon icon="grommet-icons:next" width="18" height="18"></iconify-icon>
    <span class="tag-line en">สาย Enoden</span>
    <span class="tag-duration">เดินทางประมาณ 1 ชั่วโมง 34 นาที</span> 
    ถึงสถานี 
    <a href="https://www.enoden.co.jp/en/train/station/kamakurakokomae/" target="_blank" class="detail-map">
    Kamakurakōkō-Mae Station 
    </a>
    <span class="tag-time">เวลา 16:12</span>
    <a href="https://www.google.com/maps/dir/?api=1&origin=Tokyo+Station&destination=Kamakurakōkō-Mae+Station&travelmode=transit" class="open-map">
    เปิดเส้นทาง GoogleMap
    </a>
    </div>

    <hr class="line">

    <div>เดินถ่ายรูป &nbsp;
    <iconify-icon icon="ion:camera-outline" width="30" height="30"></iconify-icon>
    <span class="tag-duration">ประมาณ 1 ชั่วโมง </span>
    <span class="tag-time">เวลา 17:12</span>
    </div>
    
    <hr class="line">
    
    <a href="https://www.google.com/maps/search/?api=1&query=food+Kamakurakōkō-Mae+Station">
    <div>
    <i>ทานอาหาร </i>
    <iconify-icon icon="fluent:food-20-regular" width="30" height="30"></iconify-icon>
    <span class="tag-duration">ประมาณ 1 ชั่วโมง </span>
    <span class="tag-time">เวลา 18:12</span>
    </div>
    </a>
    
    <hr class="line">

    <div>เดินไปขึ้นรถไฟ
    <iconify-icon icon="tabler:walk" width="30" height="30"></iconify-icon>
    <iconify-icon icon="ph:train" width="30" height="30"></iconify-icon>
    <span class="tag-duration">ประมาณ 30 นาที </span> 
    <span class="tag-time">เวลา 18:42</span> </div>

    <hr class="line">

    <div>
    <span class="tag-time">เวลา 19:00</span>
    ขึ้นรถไฟสถานี 
    <a href="https://www.enoden.co.jp/en/train/station/shichirigahama/" target="_blank" class="detail-map">
    Shichirigahama Station
    </a>
    <span class="tag-line en">สาย Enoden </span>
    <a href="https://www.enoden.co.jp/en/train/station/fujisawa/" target="_blank" class="detail-map">
    แผนที่ Fujisawa Station ต่อขบวน ไปขึ้นสาย
    </a>
    <iconify-icon icon="grommet-icons:next" width="18" height="18"></iconify-icon>
    <span class="tag-line jt">สาย Tōkaidō</span>
    <span class="tag-duration">เดินทางประมาณ 1 ชั่วโมง 21 นาที</span> 
    ถึงสถานี 
    <a href="https://www.jreast.co.jp/e/stations/e1039.html" target="_blank" class="detail-map">
    Tokyo Station 
    </a>
    <span class="tag-time">เวลา 20:18</span>
    <a href="https://www.google.com/maps/dir/?api=1&origin=Shichirigahama+Station&destination=Tokyo+Station&travelmode=transit" class="open-map">
    เปิดเส้นทาง GoogleMap
    </a>
    </div>
    
    <hr class="line">

    <a href="https://www.jreast.co.jp/e/customer_support/service_center_tokyo.html" target="_blank">
    <div>
    รับตั๋ว &nbsp;
    <iconify-icon icon="uil:ticket" width="30" height="30"></iconify-icon> &nbsp;
    <i> JR Tokyo Wind Pass ที่ตู้อัตโนมัติ </i>
    <span class="tag-duration">ประมาณ 30 นาที </span> 
    <span class="tag-time">เวลา 20:30</span> </div>
    </a>

    <hr class="line">

    <div>ไปรับกระเป๋าที่ฝาก
    <iconify-icon icon="mdi:bag-suitcase-outline" width="30" height="30"></iconify-icon>
    <span class="tag-duration">ประมาณ 30 นาที </span> 
    <span class="tag-time">เวลา 21:00</span> </div>

    <hr class="line">

    <div>
    <span class="tag-time">เวลา 21:20</span>
    ขึ้นรถไฟสถานี 
    <a href="https://www.jreast.co.jp/e/stations/e1039.html" target="_blank" class="detail-map">
    Tokyo Station 
    </a>
    <span class="tag-line ueno">สาย Ueno-Tokyo</span>
    <span class="tag-duration">เดินทางประมาณ 15 นาที</span> 
    ถึงสถานี 
    <a href="https://www.jreast.co.jp/e/stations/e34.html" target="_blank" class="detail-map">
    Akabane Station 
    </a>
    <span class="tag-time">เวลา 21:36</span>
    <a href="https://www.google.com/maps/dir/?api=1&origin=Tokyo+Station&destination=Akabane+Station&travelmode=transit" class="open-map">
    เปิดเส้นทาง GoogleMap
    </a>
    </div>

    <hr class="line">

    <a href="https://www.google.com/maps/search/?api=1&query=Toyoko-Inn+東横INN赤羽駅東口">
    <div>เดินจากสถานีไปที่พัก 
    <iconify-icon icon="tabler:walk" width="30" height="30"></iconify-icon>
    <iconify-icon icon="fontisto:hotel" width="30" height="30"></iconify-icon>
    <span class="tag-duration">ประมาณ 40 นาที </span> 
    ถึงที่พักประมาณ
    <span class="tag-time">เวลา 22:16</span> 
    <i>Check in ที่พัก Toyoko Inn Tokyo Akabane-eki Higashi-guchi</i>
    </div>
    </a>

    <hr class="line">

    <div> พรุ่งนี้ตื่นตี 5 </div>
    `,
    locations: [
      {
        location_name: "Kamakurakōkō-Mae Station",
        picture: KamakurakokoMaeImage,
        path: "/locations/kamakurakoko_mae_station",
      },
    ],
  },

  /* --------------------------------------------------- 25 ---------------------------------------------------- */

  {
    day: "Monday, 25 Mar. 2024",
    fromtime: "06:32",
    planfrom: "ยืนอยู่สถานี Akabane Station",
    duration: "อยู่บนรถไฟ 10 นาที",
    planto: "ถึงสถานี Ikebukuro Station",
    totime: "06:40",
    name: "Saitama",
    detailmore: `
    
    <div>
    <span class="tag-time">เวลา 06:32</span>
    ขึ้นรถไฟสถานี 
    <a href="https://www.jreast.co.jp/e/stations/e108.html" target="_blank" class="detail-map">
    Akabane Station 
    </a>
    <span class="tag-line ja">สาย Saikyō</span>
    <span class="tag-duration">เดินทางประมาณ 10 นาที</span> 
    ถึงสถานี 
    <a href="https://www.jreast.co.jp/e/stations/e108.html" target="_blank" class="detail-map">
    Ikebukuro Station 
    </a>
    <span class="tag-time">เวลา 06:40</span>
    <a href="https://www.google.com/maps/dir/?api=1&origin=Akabane+Station&destination=Ikebukuro+Station&travelmode=transit" class="open-map">
    เปิดเส้นทาง GoogleMap
    </a>
    </div>

    <hr class="line">

    <a href="https://www.tobu.co.jp/en/ticket/kawagoe/premium.html" target="_blank">
    <div>
    ซื้อตั๋ว &nbsp;
    <iconify-icon icon="uil:ticket" width="30" height="30"></iconify-icon> &nbsp;
    <i>KAWAGOE DISCOUNT PASS Premium 2 ใบ</i>
    </div>
    </a>

    <hr class="line">

    <div>
    <span class="tag-time">เวลา 07:15</span>
    ขึ้นรถไฟสถานี 
    <a href="https://www.tobu.co.jp/th/service/station/7105.html" target="_blank" class="detail-map">
    Ikebukuro Station 
    </a>
    <span class="tag-line tj">สาย Tobu-Tojo</span>
    <span class="tag-duration">เดินทางประมาณ 40 นาที</span> 
    ถึงสถานี 
    <a href="https://www.tobu.co.jp/th/service/station/7315.html" target="_blank" class="detail-map">
    Kawagoe Station 
    </a>
    <span class="tag-time">เวลา 07:49</span>
    <a href="https://www.google.com/maps/dir/?api=1&origin=Ikebukuro+Station&destination=Kawagoe+Station&travelmode=transit" class="open-map">
    เปิดเส้นทาง GoogleMap
    </a>
    </div>

    <hr class="line">

    <a href="https://www.google.com/maps/search/?api=1&query=food+Kawagoe+Station">
    <div>
    <i>ทานอาหาร </i>
    <iconify-icon icon="fluent:food-20-regular" width="30" height="30"></iconify-icon>
    <span class="tag-duration">ประมาณ 1 ชั่วโมง </span> 
    <span class="tag-time">เวลา 08:50</span></div>
    </a>

    <hr class="line">

    <div>เดินไปขึ้นรถบัส
    <iconify-icon icon="tabler:walk" width="30" height="30"></iconify-icon>
    <iconify-icon icon="tabler:bus" width="30" height="30"></iconify-icon>
    <span class="tag-duration">ประมาณ 10 นาที </span> 
    </div>

    <hr class="line">
  
    <a href="https://www.tobu-bus.com/en/kawagoe/" target="_blank">
    <div>
    <i>ลงป้าย</i> &nbsp;
    <iconify-icon icon="ant-design:stop-twotone" width="30" height="30"></iconify-icon>
    <i>W5 Renkeiji Temple 蓮馨寺 </i>
    </div>
    </a>


    <hr class="line">

    <div>
    เดินไปร้าน
    <a href="https://vasara-h.co.jp/en/" target="_blank">
    <i>Kimono Rental VASARA Kawagoe Koedo Store</i>
    </a>
    <iconify-icon icon="tabler:walk" width="30" height="30"></iconify-icon>
    <iconify-icon icon="clarity:store-solid" width="30" height="30"></iconify-icon>
    <span class="tag-duration">ประมาณ 200 เมตร</span> 
    จองชุดร้านไว้ตอน 10 โมง
    <a href="https://www.google.com/maps/dir/?api=1&origin=蓮馨寺（バス）&destination=Kimono+Rental+VASARA+Kawagoe+Koedo+Store&travelmode=walking" class="open-map">
    เปิดเส้นทาง GoogleMap
    </a>
    </div>

    <hr class="line">

    `,
    locations: [
      {
        location_name: "Kurazukuri no Machinami",
        picture: KurazukurinoMachinamiImage,
        path: "/locations/kurazukuri_no_machinami",
      },
      {
        location_name: "Kawagoe Hikawa Shrine",
        picture: KawagoeHikawaShrineImage,
        path: "/locations/kawagoe_hikawa_shrine",
      },
      {
        location_name: "Shingashi Riverbank",
        picture: ShingashiRiverbankImage,
        path: "/locations/shingashi_riverbank",
      },
    ],
  },

  /* ------------------------------------------------- 26 ------------------------------------------------------ */

  {
    day: "Tuseday, 26 Mar. 2024",
    fromtime: "06:03",
    planfrom: "ยืนอยู่สถานี Akabane Station",
    duration: "อยู่บนรถไฟ 15 นาที",
    planto: "ถึงสถานี Shinjuku Station",
    totime: "06:18",
    name: "Fujikawaguchiko, Yamanashi",
    detailmore: `
    <div>
    <span class="tag-time">เวลา 06:03</span>
    ขึ้นรถไฟสถานี 
    <a href="https://www.jreast.co.jp/e/stations/e108.html" target="_blank" class="detail-map">
    Akabane Station 
    </a>
    <span class="tag-line ja">สาย Saikyō</span>
    <span class="tag-duration">เดินทางประมาณ 15 นาที</span> 
    ถึงสถานี 
    <a href="https://www.jreast.co.jp/e/stations/e866.html" target="_blank" class="detail-map">
    Shinjuku Station
    </a>
    <span class="tag-time">เวลา 06:18</span>
    <a href="https://www.google.com/maps/dir/?api=1&origin=Akabane+Station&destination=Shinjuku+Station&travelmode=transit" class="open-map">
    เปิดเส้นทาง GoogleMap
    </a>
    </div>

    <hr class="line">

    <a href="https://www.google.com/maps/search/?api=1&query=food+Shinjuku+Station">
    <div>
    <i>ซื้อข้าวกล่องไปกินบนรถไฟ</i> &nbsp;
    <iconify-icon icon="mdi:cash" width="30" height="30"></iconify-icon>
    <iconify-icon icon="fluent:food-20-regular" width="30" height="30"></iconify-icon>
    </div>
    </a>

    <hr class="line">

    <div>
    <span class="tag-time">เวลา 07:30</span>
    ขึ้นรถไฟสถานี 
    <a href="https://www.jreast.co.jp/e/stations/e866.html" target="_blank" class="detail-map">
    Shinjuku Station
    </a>
    <span class="tag-line jb">สาย Chūō-Sōbu</span>
    ต่อบนพาหนะเดิม
    <span class="tag-line jc">สาย Chūō</span>
    ต่อบนพาหนะเดิม
    <span class="tag-line fuji">สาย Fujikyuko No.3</span>
    <a href="https://www.jreast.co.jp/hachioji/fuji_excursion/en/#fuji_station" target="_blank" class="detail-map">
    ข้อมูลสาย FUJI EXCURSION
    </a>
    <span class="tag-duration">เดินทางประมาณ 1 ชั่วโมง 56 นาที</span> 
    <span class="detail-map">
    ถึงสถานี Kawaguchiko Station
    </span>
    <span class="tag-time">เวลา 09:26</span>
    <a href="https://www.google.com/maps/dir/?api=1&origin=Shinjuku+Station&destination=Kawaguchiko+Station&travelmode=transit" class="open-map">
    เปิดเส้นทาง GoogleMap
    </a>
    </div>

    <hr class="line">

    <div>เดินไปขึ้นรถบัส สายสี Red
    <iconify-icon icon="tabler:walk" width="30" height="30"></iconify-icon>
    <iconify-icon icon="tabler:bus" width="30" height="30"></iconify-icon>

    <a href="https://en.kawaguchiko.net/img/pdf/bus_timetable_en.pdf" target="_blank" class="detail-map">
    เวลารถบัส ออกทุก 15 นาที
    </a>

    </div>

    <hr class="line">

    <a href="#" target="_blank">
    <div>
    <i>ลงป้าย</i> &nbsp;
    <iconify-icon icon="ant-design:stop-twotone" width="30" height="30"></iconify-icon>
    <i>Shimazuyashiki-mae No.18 </i>
    </div>
    </a>

    <hr class="line">

    <div>เดินไปถ่ายรูปที่ Nagasaki Park &nbsp;
    <iconify-icon icon="tabler:walk" width="30" height="30"></iconify-icon>
    <iconify-icon icon="ion:camera-outline" width="30" height="30"></iconify-icon>
    <span class="tag-duration">ระยะทางเดิน 300 เมตร</span>
    <a href="https://www.google.com/maps/dir/?api=1&origin=Shimazuyashiki-mae&destination=Nagasaki+Park,+Oishi&travelmode=walking" class="open-map">
    เปิดเส้นทาง GoogleMap
    </a>
    </div>

    <hr class="line">

    <div>เดินไป 
    <iconify-icon icon="tabler:walk" width="30" height="30"></iconify-icon>
    Oishi Park Café ผ่าน&nbsp;
    <a href="https://www.google.com/maps/search/?api=1&query=Lake+Bake+Cafe">
    <i>ร้านขนมปัง Lake Bake Cafe</i>
    </a>
    <iconify-icon icon="mdi:bread" width="30" height="30"></iconify-icon>
    <iconify-icon icon="ph:coffee" width="30" height="30"></iconify-icon>
    <span class="tag-duration">ระยะทางเดิน 1.5 กิโลเมตร เดินประมาณ 1 ชั่วโมง</span>
    <a href="https://www.google.com/maps/dir/?api=1&origin=Nagasaki+Park,+Oishi&destination=Oishi+Park+Café&travelmode=walking" class="open-map">
    เปิดเส้นทาง GoogleMap
    </a>
    </div>

    <hr class="line">

    <div>เดินไปขึ้นรถบัสเตรียมกลับสถานี Kawaguchiko Station
    <iconify-icon icon="tabler:walk" width="30" height="30"></iconify-icon>
    <iconify-icon icon="tabler:bus" width="30" height="30"></iconify-icon>
    <span class="tag-duration">เวลา 13:30 ควรยืนรอขึ้นรถบัส  </span> 
    </div>

    <hr class="line">

    <div>
    <span class="tag-time">เวลา 15:03</span>
    <span class="detail-map">
    ขึ้นรถไฟสถานี Kawaguchiko Station
    </span>
    <span class="tag-line fuji">สาย Fujikyuko No.36</span>
    <a href="https://www.jreast.co.jp/hachioji/fuji_excursion/en/#fuji_station" target="_blank" class="detail-map">
    ข้อมูลสาย FUJI EXCURSION
    </a>
    <span class="tag-line jc">สาย Chūō</span>
    ต่อบนพาหนะเดิม
    <span class="tag-line jc">สาย Chūō</span>
    ต่อบนพาหนะเดิม
    <span class="tag-duration">เดินทางประมาณ 1 ชั่วโมง 54 นาที</span> 
    ถึงสถานี 
    <a href="https://www.jreast.co.jp/e/stations/e866.html" target="_blank" class="detail-map">
    Shinjuku Station
    </a>
    <span class="tag-time">เวลา 16:57</span>
    <a href="https://www.google.com/maps/dir/?api=1&origin=Kawaguchiko+Station&destination=Shinjuku+Station&travelmode=transit" class="open-map">
    เปิดเส้นทาง GoogleMap
    </a>
    </div>
    
    <hr class="line">

    <div>เดินไปขึ้นรถไฟ
    <iconify-icon icon="tabler:walk" width="30" height="30"></iconify-icon>
    <iconify-icon icon="ph:train" width="30" height="30"></iconify-icon>
    <span class="tag-duration">ประมาณ 7 นาที 450 เมตร</span> 
    <a href="https://www.google.com/maps/dir/?api=1&origin=Shinjuku+Station&destination=Shinjuku-sanchome+Station&travelmode=walking" class="open-map">
    เปิดเส้นทาง GoogleMap
    </a>
    </div>

    <hr class="line">

    <div>
    <span class="tag-time">เวลา 17:12</span>
    ขึ้นรถไฟสถานี 
    <a href="https://www.tokyometro.jp/lang_th/station/shinjuku-sanchome/index.html" target="_blank" class="detail-map">
    Shinjuku-sanchome Station
    </a>
    <span class="tag-line f">สาย Fukutoshin</span>
    ต่อบนพาหนะเดิม
    <span class="tag-line ty">สาย Tōyoko</span>
    <span class="tag-duration">เดินทางประมาณ 11 นาที</span> 
    ถึงสถานี 
    <a href="https://www.tokyometro.jp/lang_th/station/naka-meguro/index.html" target="_blank" class="detail-map">
    Naka-meguro Sta.
    </a>
    <span class="tag-time">เวลา 17:23</span>
    <a href="https://www.google.com/maps/dir/?api=1&origin=Shinjuku-sanchome+Station&destination=Naka-meguro+Sta.&travelmode=transit" class="open-map">
    เปิดเส้นทาง GoogleMap
    </a>
    </div>
    
    <hr class="line">

    <div>เดินถ่ายรูป ชมซากุระ &nbsp;
    <iconify-icon icon="tabler:walk" width="30" height="30"></iconify-icon>
    <iconify-icon icon="ion:camera-outline" width="30" height="30"></iconify-icon>
    </div>

    <hr class="line">

    <div>
    กลับที่พัก มีทางเลือกสองทาง
    </div>   

    <div>
    1) เดิน 
    <span class="tag-duration">ระยะทาง 1.3 กิโลเมตร ประมาณ 20 นาที </span> 
    ไปสถานี 
    <a href="https://www.jreast.co.jp/e/stations/e290.html" target="_blank" class="detail-map">
    Ebisu Station
    </a>
    เพื่อขึ้น
    <span class="tag-line ja">สาย Saikyō</span>
    <span class="tag-time">เวลา </span> 
    <a href="https://www.google.com/maps/search/?api=1&query=Ebisu+Station" class="open-map">
    เปิดเส้นทางที่ 1 บน GoogleMap 
    </a>
    </div>   

   
    <div>
    2) เดิน
    <span class="tag-duration">ระยะทาง  กิโลเมตร ประมาณ  นาที </span> 
    ไปสถานี
    <a href="https://www.tokyometro.jp/lang_th/station/naka-meguro/index.html" target="_blank" class="detail-map">
    Naka-meguro Sta. 
    </a>
    ขึ้นรถไฟ
    <span class="tag-line h">สาย Hibiya</span>
    ลงสถานี
    <a href="https://www.tokyometro.jp/lang_th/station/ebisu/index.html" target="_blank" class="detail-map">
    Ebisu Station
    </a>
    เดินไป
    <iconify-icon icon="tabler:walk" width="30" height="30"></iconify-icon>
    ขึ้นรถไฟ 
    <a href="https://www.jreast.co.jp/e/stations/e290.html" target="_blank" class="detail-map">
    Ebisu 
    </a>
    <span class="tag-line ja">สาย Saikyō</span>
    <a href="https://www.google.com/maps/dir/?api=1&origin=+Naka-meguro+Sta.&destination=Ebisu+Station&travelmode=transit" class="open-map">
    เปิดเส้นทางที่ 2 บน GoogleMap
    </a>
    </div>   

    <hr class="line">


    <div>
    <span class="tag-time">เวลา </span>
    ไปสถานี 
    <a href="https://www.jreast.co.jp/e/stations/e34.html" target="_blank" class="detail-map">
    Akabane Station 
    </a>
    <span class="tag-time">เวลา </span>
    <a href="https://www.google.com/maps/search/?api=1&query=Akabane+Station" class="open-map">
    เปิดเส้นทาง GoogleMap
    </a>
    </div>

    <hr class="line">

    <a href="https://www.google.com/maps/search/?api=1&query=Toyoko-Inn+東横INN赤羽駅東口">
    <div>เดินจากสถานีไปที่พัก 
    <iconify-icon icon="tabler:walk" width="30" height="30"></iconify-icon>
    <iconify-icon icon="fontisto:hotel" width="30" height="30"></iconify-icon>
    <span class="tag-duration">ประมาณ 40 นาที </span> 
    ถึงที่พักประมาณ
    <span class="tag-time">เวลา </span> 
    <i>Check in ที่พัก Toyoko Inn Tokyo Akabane-eki Higashi-guchi</i>
    </div>
    </a>

    <hr class="line">

    <div> พรุ่งนี้ตื่นตี 5 </div>


    `,
    locations: [
      {
        location_name: "Oishi Park Cafe",
        picture: OishiParkCafeImage,
        path: "/locations/oishi_park_cafe",
      },
      {
        location_name: "Naka-meguro Sta.",
        picture: NakaMeguroStaImage,
        path: "/locations/naka_meguro_sta",
      },
    ],
  },

  /* ---------------------------------------------------------- 27 --------------------------------------------- */

  {
    day: "Wednesday, 27 Mar. 2024",
    fromtime: "0",
    planfrom: "ยืนอยู่สถานี Akabane Station",
    duration: "อยู่บนรถไฟ นาที",
    planto: "ถึงสถานี Tokyo Station",
    totime: "0",
    name: "Niigata and Saitama",
    detailmore: `
    <div>
    <span class="tag-time">เวลา 06:03</span>
    ขึ้นรถไฟสถานี 
    <a href="https://www.jreast.co.jp/e/stations/e34.html" target="_blank" class="detail-map">
    Akabane Station 
    </a>
    <span class="tag-line ja">สาย </span>
    <span class="tag-duration">เดินทางประมาณ 15 นาที</span> 
    ถึงสถานี 
    <a href="https://www.jreast.co.jp/e/stations/e1039.html" target="_blank" class="detail-map">
    Tokyo Station
    </a>
    <span class="tag-time">เวลา 06:18</span>
    <a href="https://www.google.com/maps/dir/?api=1&origin=Akabane+Station&destination=Tokyo+Station&travelmode=transit" class="open-map">
    เปิดเส้นทาง GoogleMap
    </a>
    </div>

    <hr class="line">

    <div>
    <span class="tag-time">เวลา </span>
    ขึ้นรถไฟสถานี 
    <a href="" target="_blank" class="detail-map">
    Tokyo Station
    </a>
    <span class="tag-line ja">สาย </span>
    <span class="tag-duration">เดินทางประมาณ  นาที</span> 
    ถึงสถานี 
    <a href="" target="_blank" class="detail-map">
    Gala Yuzawa
    </a>
    <span class="tag-time">เวลา </span>
    <a href="https://www.google.com/maps/dir/?api=1&origin=Tokyo+Station&destination=     &travelmode=transit" class="open-map">
    เปิดเส้นทาง GoogleMap
    </a>
    </div>

    <hr class="line">

    <div>
    <a href="https://scontent.fbkk26-1.fna.fbcdn.net/v/t39.30808-6/412847057_356377887088495_732512912705972435_n.jpg?_nc_cat=107&ccb=1-7&_nc_sid=dd5e9f&_nc_eui2=AeF8WlvsqHIkTIoLAeZjmvKWO65VAi38dtg7rlUCLfx22E1d7nNXkaoPB-2tRKCTCWb94T7_QUgLlKtONuL6xPtB&_nc_ohc=uw_n-IYZ5-cAX84xNau&_nc_ht=scontent.fbkk26-1.fna&oh=00_AfArPewIcMgYGkVd5c99WGlm6iUcdiHjhtnp7TXxQ1Tr4g&oe=65A59375" target="_blank">
    <i>ซื้อตั๋วกอนโดล่า &nbsp;</i>
    </a>
    <iconify-icon icon="uil:ticket" width="30" height="30"></iconify-icon> &nbsp;
    <i>2 ใบ</i>
    </div>
    <div>กับ</div>
    <div>
    <a href="https://gala.co.jp/winter/thai/rental-thai/" target="_blank">
    <i>เช่าชุด</i>
    </a>
    </div>
    <div>
    เก็บของที่ตู้ล็อคเกอร์
    </div>
    <div>
    <a href="https://gala.co.jp/winter/thai/" target="_blank">
    <i>ดูข้อมูลเว็บของ Gala</i>
    </a>
    </div>
    

    <hr class="line">
    
    <div>ขึ้นกอดโดล่าเพื่อไปถ่ายรูปกับหิมะ &nbsp;
    <iconify-icon icon="ion:camera-outline" width="30" height="30"></iconify-icon>
    <span class="tag-duration">ประมาณ 1 ชั่วโมง </span>
    <span class="tag-time">เวลา</span>
    </div>

    <hr class="line">


    <div>
    <u>1) แผนหลัก</u>
    </div>

    <div>
    <span class="tag-time">เวลา </span>
    ขึ้นรถไฟสถานี 
    <a href="" target="_blank" class="detail-map">
    Gala Yuzawa
    </a>
    <span class="tag-line ja">สาย </span>
    <span class="tag-duration">เดินทางประมาณ  นาที</span> 
    ถึงสถานี 
    <a href="https://www.jreast.co.jp/e/stations/e625.html" target="_blank" class="detail-map">
    Kumagaya Station
    </a>
    <span class="tag-time">เวลา </span>
    <a href="https://www.google.com/maps/dir/?api=1&origin=    &destination=Kumagaya+Station&travelmode=transit" class="open-map">
    เปิดเส้นทาง GoogleMap
    </a>
    </div>

    <div>เดินถ่ายรูปที่ 
    <a href="https://www.google.com/maps/search/?api=1&query=Kumagaya+Sakura+Tsutsumi">
    <i>Kumagaya Sakura Tsutsumi</i> 
    </a> &nbsp;
    <iconify-icon icon="ion:camera-outline" width="30" height="30"></iconify-icon>
    <span class="tag-duration">ประมาณ 1 ชั่วโมง </span>
    <span class="tag-time">เวลา </span>
    </div>

    <div>
    <u>2) แผนสำรอง</u>
    </div>

    <div>
    <span class="tag-time">เวลา </span>
    ขึ้นรถไฟสถานี 
    <a href="" target="_blank" class="detail-map">
    Gala Yuzawa
    </a>
    <span class="tag-line ja">สาย </span>
    <span class="tag-duration">เดินทางประมาณ  นาที</span> 
    ถึงสถานี 
    <a href="https://www.jreast.co.jp/e/stations/e350.html" target="_blank" class="detail-map">
    Ōmiya Station
    </a>
    <span class="tag-time">เวลา </span>
    <a href="https://www.google.com/maps/dir/?api=1&origin=    &destination=Ōmiya+Station&travelmode=transit" class="open-map">
    เปิดเส้นทาง GoogleMap
    </a>
    </div>

    <div>เดินถ่ายรูปที่ 
    <a href="https://www.google.com/maps/search/?api=1&query=Omiya+Parki">
    <i>Omiya Park</i> 
    </a> &nbsp;
    <iconify-icon icon="ion:camera-outline" width="30" height="30"></iconify-icon>
    <span class="tag-duration">ประมาณ 1 ชั่วโมง </span>
    <span class="tag-time">เวลา </span>
    </div>

    <div>
    <a href="https://www.google.com/maps/search/?api=1&query=food+Omiya+Station">
    <i>ดูข้อมูลร้านอาหารแถว Omiya</i>
    </a>
    <iconify-icon icon="fluent:food-20-regular" width="30" height="30"></iconify-icon>
    </div>

    <hr class="line">

    <div>
    <a href="https://www.google.com/maps/search/?api=1&query=Akabane+Station">
    <i>ไปสถานี Akabane Station กลับที่พัก</i>
    </a>
    </div>
    `,
    locations: [
      {
        location_name: "Gala Yuzawa",
        picture: GalaYuzawaImage,
        path: "/locations/gala_yuzawa",
      },
      {
        location_name: "Kumagaya Sakura Tsutsumi",
        picture: "",
        path: "/locations/kumagaya_sakura_tsutsumi",
      },
    ],
  },

  /* ------------------------------------------------------------------------------------------------------- */

  {
    day: "Thursday, 28 Mar. 2024",
    fromtime: "06:26",
    planfrom: "ยืนอยู่สถานี Akabane Station",
    duration: "อยู่บนรถไฟ นาที",
    planto: "ถึงสถานี Tokyo Station",
    totime: "06:43",
    name: "Kanagawa",
    detailmore: `
    <div>
    Check Out ออกจากที่พัก 
    </div>

    <hr class="line">

    <div>
    <span class="tag-time">เวลา 06:26</span>
    ขึ้นรถไฟสถานี 
    <a href="https://www.jreast.co.jp/e/stations/e34.html" target="_blank" class="detail-map">
    Akabane Station 
    </a>
    <span class="tag-line ueno">สาย Ueno-Tokyo</span>
    <span class="tag-duration">เดินทางประมาณ 17 นาที</span> 
    ถึงสถานี 
    <a href="https://www.jreast.co.jp/e/stations/e1039.html" target="_blank" class="detail-map">
    Tokyo Station
    </a>
    <span class="tag-time">เวลา 06:43</span>
    <a href="https://www.google.com/maps/dir/?api=1&origin=Akabane+Station&destination=Tokyo+Station&travelmode=transit" class="open-map">
    เปิดเส้นทาง GoogleMap
    </a>
    </div>

    <hr class="line">

    <div>
    เดินไปฝากกระเป๋าตู้ล็อกเกอร์
    </div>

    <hr class="line">
    
    <div>
    <span class="tag-time">เวลา 07:09</span>
    ขึ้นรถไฟสถานี 
    <a href="https://www.jreast.co.jp/e/stations/e1039.html" target="_blank" class="detail-map">
    Tokyo Station
    </a>
    <span class="tag-line ueno">สาย Ueno-Tokyo</span>
    ลงสถานี 
    <a href="https://www.jreast.co.jp/e/stations/e526.html" target="_blank" class="detail-map">
    Kawasaki Station
    </a>
    เดินไปเปลี่ยนสายขึ้น
    <span class="tag-line jn">
    สาย Nambu
    </span>
    <span class="tag-duration">เดินทางประมาณ 2 นาที</span> 
    ถึงสถานี 
    <a href="https://www.jreast.co.jp/estation/stations/839.html" target="_blank" class="detail-map">
    Shukugawara Station
    </a>
    <span class="tag-time">เวลา 07:59</span>
    <a href="https://www.google.com/maps/dir/?api=1&origin=Akabane+Station&destination=Tokyo+Station&travelmode=transit" class="open-map">
    เปิดเส้นทาง GoogleMap
    </a>
    </div>

    <hr class="line">

    <div>เดินถ่ายรูป
    <a href="https://www.google.com/maps/search/?api=1&query=Shukugawara+Nikaryō+Waterway">
    <i>Shukugawara Nikaryō Waterway &nbsp;</i>
    </a>
    <iconify-icon icon="ion:camera-outline" width="30" height="30"></iconify-icon>
    <span class="tag-duration">ประมาณ 1 ชั่วโมง </span>
    <span class="tag-time">เวลา </span>
    </div>


    `,
    locations: [
      {
        location_name: "Shukugawara Nikaryō Waterway",
        picture: "",
        path: "/locations/shukugawara_nikaryo_waterway",
      },
    ],
  },
];

const travelData = [
  {
    day: "Sunday, 24 Mar. 2024",
    location_name: "Kamakurakōkō-Mae Station",
  },
  {
    day: "Monday, 25 Mar. 2024",
    location_name: "Kamakurakōkō-Mae Station",
  },
  {
    day: "Tuseday, 26 Mar. 2024",
    location_name: "Kamakurakōkō-Mae Station",
  },
  {
    day: "Wednesday, 27 Mar. 2024",
    location_name: "Kamakurakōkō-Mae Station",
  },
  {
    day: "Thursday, 28 Mar. 2024",
    location_name: "Kamakurakōkō-Mae Station",
  },
];

export { swiperData, travelData };
